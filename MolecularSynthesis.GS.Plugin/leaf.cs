...
{
    public class MCTS_Parallel_leaf : SearchProcess
    {
        // give desiredMoment
        ...
        public MCTS_Parallel_leaf(GlobalSettings settings) : base(settings)
        {
            RequireSeed = true;
        ...
        public double Rollout(TreeCandidate candidate)
        {
            ...
            //for (int i = 0; i < 10; i++)
            {
                ...
                {
                    //rnd.Next(0, 2); // generate 0 or 1
                    List<option> option0 = null;
                    lock (child.graph)
                        option0 = rulesets[0].recognize(child.graph, false);
                    ...
                    {
                        List<option> option1 = null;
                        lock (child.graph)
                            option1 = rulesets[1].recognize(child.graph, false);
                        if (option1.Count > 0)
                        {
                            RS1 = RS1 + 1;
                ...
                var option2 = rulesets[2].recognize(child.graph, false);
